# -*- coding: utf-8 -*-
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: md_energy.proto
"""Generated protocol buffer code."""
from google.protobuf import descriptor as _descriptor
from google.protobuf import descriptor_pool as _descriptor_pool
from google.protobuf import message as _message
from google.protobuf import reflection as _reflection
from google.protobuf import symbol_database as _symbol_database
# @@protoc_insertion_point(imports)

_sym_db = _symbol_database.Default()




DESCRIPTOR = _descriptor_pool.Default().AddSerializedFile(b'\n\x0fmd_energy.proto\x12\x0b\x64\x62\x61g.energy\"\xa5\x01\n\x10MarketDataReport\x12\x30\n\x03\x45vt\x18\x01 \x01(\x0e\x32#.dbag.energy.MarketDataReport.Event\x12\x0b\n\x03\x43nt\x18\x02 \x01(\x05\"R\n\x05\x45vent\x12\x11\n\rUNKNOWN_EVENT\x10\x00\x12\x1b\n\x17START_OF_REFERENCE_DATA\x10\x01\x12\x19\n\x15\x45ND_OF_REFERENCE_DATA\x10\x02\"\xf6\x01\n\nInstrument\x12\x0b\n\x03Seq\x18\x01 \x01(\x05\x12\n\n\x02ID\x18\x02 \x01(\x03\x12\x0c\n\x04\x42\x61se\x18\x03 \x01(\t\x12\x11\n\tCmdtyType\x18\x04 \x01(\t\x12\x0c\n\x04\x44\x65sc\x18\x05 \x01(\t\x12\r\n\x05Start\x18\x06 \x01(\x04\x12\x0b\n\x03\x45nd\x18\x07 \x01(\x04\x12\x0e\n\x06Period\x18\x08 \x01(\t\x12\x0b\n\x03UOM\x18\t \x01(\t\x12\x0c\n\x04SIID\x18\n \x01(\x05\x12\x0c\n\x04SSID\x18\x0b \x01(\x05\x12\x0c\n\x04\x46IID\x18\x0c \x01(\x05\x12\x0c\n\x04SQID\x18\r \x01(\x05\x12\x0f\n\x07\x45ncDesc\x18\x0e \x01(\t\x12\x0f\n\x07StartTm\x18\x0f \x01(\x04\x12\r\n\x05\x45ndTm\x18\x10 \x01(\x04\"\x93\x01\n\x13MDUpdateActionValue\x12>\n\x05Value\x18\x01 \x01(\x0e\x32/.dbag.energy.MDUpdateActionValue.MDUpdateAction\"<\n\x0eMDUpdateAction\x12\x07\n\x03NEW\x10\x00\x12\n\n\x06\x43HANGE\x10\x01\x12\n\n\x06\x44\x45LETE\x10\x02\x12\t\n\x05QUERY\x10\x06\"\xc8\x07\n\x05Order\x12\n\n\x02ID\x18\x01 \x01(\x03\x12\x0c\n\x04MDID\x18\x02 \x01(\t\x12+\n\x03Typ\x18\x03 \x01(\x0e\x32\x1e.dbag.energy.Order.MDEntryType\x12\x31\n\x07UpdtAct\x18\x04 \x01(\x0b\x32 .dbag.energy.MDUpdateActionValue\x12\n\n\x02Px\x18\x05 \x01(\x01\x12\n\n\x02Sz\x18\x06 \x01(\x01\x12\x35\n\x08MDQteTyp\x18\x07 \x01(\x0b\x32#.dbag.energy.Order.MDQuoteTypeValue\x12\x36\n\tTmInForce\x18\x08 \x01(\x0b\x32#.dbag.energy.Order.TimeInForceValue\x12>\n\x0bImpldMktInd\x18\t \x01(\x0e\x32).dbag.energy.Order.ImpliedMarketIndicator\x12\x39\n\x08\x45xecInst\x18\n \x01(\x0e\x32\'.dbag.energy.Order.ExecutionInstruction\x12\n\n\x02Tm\x18\x63 \x01(\x04\x1a\xa1\x01\n\x10MDQuoteTypeValue\x12>\n\x05Value\x18\x01 \x01(\x0e\x32/.dbag.energy.Order.MDQuoteTypeValue.MDQuoteType\"M\n\x0bMDQuoteType\x12\x0e\n\nINDICATIVE\x10\x00\x12\x08\n\x04\x46IRM\x10\x05\x12\x0c\n\x08WITHHELD\x10\x06\x12\r\n\tREFERENCE\x10\x07\x12\x07\n\x03\x41LL\x10\x08\x1a\xcb\x01\n\x10TimeInForceValue\x12>\n\x05Value\x18\x01 \x01(\x0e\x32/.dbag.energy.Order.TimeInForceValue.TimeInForce\"w\n\x0bTimeInForce\x12\x07\n\x03\x44\x41Y\x10\x00\x12\x14\n\x10GOOD_TILL_CANCEL\x10\x01\x12\x10\n\x0c\x46ILL_OR_KILL\x10\x04\x12\x12\n\x0eGOOD_TILL_DATE\x10\x06\x12\x11\n\rFILL_AND_KILL\x10\r\x12\x10\n\x0cMARKET_ORDER\x10\x0e\"+\n\x0bMDEntryType\x12\x07\n\x03\x42ID\x10\x00\x12\t\n\x05OFFER\x10\x01\x12\x08\n\x04\x42OTH\x10\x02\"W\n\x16ImpliedMarketIndicator\x12\x0f\n\x0bNOT_IMPLIED\x10\x00\x12\n\n\x06NATIVE\x10\x05\x12\x0c\n\x08\x45XTERNAL\x10\x06\x12\x07\n\x03\x41LL\x10\x07\x12\t\n\x05VENUE\x10\x08\"?\n\x14\x45xecutionInstruction\x12\x16\n\x12STAY_ON_OFFER_SIDE\x10\x00\x12\x0f\n\x0b\x41LL_OR_NONE\x10\x01\"x\n\x05Trade\x12\n\n\x02ID\x18\x01 \x01(\x03\x12\x0c\n\x04MDID\x18\x02 \x01(\t\x12\x31\n\x07UpdtAct\x18\x03 \x01(\x0b\x32 .dbag.energy.MDUpdateActionValue\x12\n\n\x02Px\x18\x04 \x01(\x01\x12\n\n\x02Sz\x18\x05 \x01(\x01\x12\n\n\x02Tm\x18\x63 \x01(\x04\x62\x06proto3')



_MARKETDATAREPORT = DESCRIPTOR.message_types_by_name['MarketDataReport']
_INSTRUMENT = DESCRIPTOR.message_types_by_name['Instrument']
_MDUPDATEACTIONVALUE = DESCRIPTOR.message_types_by_name['MDUpdateActionValue']
_ORDER = DESCRIPTOR.message_types_by_name['Order']
_ORDER_MDQUOTETYPEVALUE = _ORDER.nested_types_by_name['MDQuoteTypeValue']
_ORDER_TIMEINFORCEVALUE = _ORDER.nested_types_by_name['TimeInForceValue']
_TRADE = DESCRIPTOR.message_types_by_name['Trade']
_MARKETDATAREPORT_EVENT = _MARKETDATAREPORT.enum_types_by_name['Event']
_MDUPDATEACTIONVALUE_MDUPDATEACTION = _MDUPDATEACTIONVALUE.enum_types_by_name['MDUpdateAction']
_ORDER_MDQUOTETYPEVALUE_MDQUOTETYPE = _ORDER_MDQUOTETYPEVALUE.enum_types_by_name['MDQuoteType']
_ORDER_TIMEINFORCEVALUE_TIMEINFORCE = _ORDER_TIMEINFORCEVALUE.enum_types_by_name['TimeInForce']
_ORDER_MDENTRYTYPE = _ORDER.enum_types_by_name['MDEntryType']
_ORDER_IMPLIEDMARKETINDICATOR = _ORDER.enum_types_by_name['ImpliedMarketIndicator']
_ORDER_EXECUTIONINSTRUCTION = _ORDER.enum_types_by_name['ExecutionInstruction']
MarketDataReport = _reflection.GeneratedProtocolMessageType('MarketDataReport', (_message.Message,), {
  'DESCRIPTOR' : _MARKETDATAREPORT,
  '__module__' : 'md_energy_pb2'
  # @@protoc_insertion_point(class_scope:dbag.energy.MarketDataReport)
  })
_sym_db.RegisterMessage(MarketDataReport)

Instrument = _reflection.GeneratedProtocolMessageType('Instrument', (_message.Message,), {
  'DESCRIPTOR' : _INSTRUMENT,
  '__module__' : 'md_energy_pb2'
  # @@protoc_insertion_point(class_scope:dbag.energy.Instrument)
  })
_sym_db.RegisterMessage(Instrument)

MDUpdateActionValue = _reflection.GeneratedProtocolMessageType('MDUpdateActionValue', (_message.Message,), {
  'DESCRIPTOR' : _MDUPDATEACTIONVALUE,
  '__module__' : 'md_energy_pb2'
  # @@protoc_insertion_point(class_scope:dbag.energy.MDUpdateActionValue)
  })
_sym_db.RegisterMessage(MDUpdateActionValue)

Order = _reflection.GeneratedProtocolMessageType('Order', (_message.Message,), {

  'MDQuoteTypeValue' : _reflection.GeneratedProtocolMessageType('MDQuoteTypeValue', (_message.Message,), {
    'DESCRIPTOR' : _ORDER_MDQUOTETYPEVALUE,
    '__module__' : 'md_energy_pb2'
    # @@protoc_insertion_point(class_scope:dbag.energy.Order.MDQuoteTypeValue)
    })
  ,

  'TimeInForceValue' : _reflection.GeneratedProtocolMessageType('TimeInForceValue', (_message.Message,), {
    'DESCRIPTOR' : _ORDER_TIMEINFORCEVALUE,
    '__module__' : 'md_energy_pb2'
    # @@protoc_insertion_point(class_scope:dbag.energy.Order.TimeInForceValue)
    })
  ,
  'DESCRIPTOR' : _ORDER,
  '__module__' : 'md_energy_pb2'
  # @@protoc_insertion_point(class_scope:dbag.energy.Order)
  })
_sym_db.RegisterMessage(Order)
_sym_db.RegisterMessage(Order.MDQuoteTypeValue)
_sym_db.RegisterMessage(Order.TimeInForceValue)

Trade = _reflection.GeneratedProtocolMessageType('Trade', (_message.Message,), {
  'DESCRIPTOR' : _TRADE,
  '__module__' : 'md_energy_pb2'
  # @@protoc_insertion_point(class_scope:dbag.energy.Trade)
  })
_sym_db.RegisterMessage(Trade)

if _descriptor._USE_C_DESCRIPTORS == False:

  DESCRIPTOR._options = None
  _MARKETDATAREPORT._serialized_start=33
  _MARKETDATAREPORT._serialized_end=198
  _MARKETDATAREPORT_EVENT._serialized_start=116
  _MARKETDATAREPORT_EVENT._serialized_end=198
  _INSTRUMENT._serialized_start=201
  _INSTRUMENT._serialized_end=447
  _MDUPDATEACTIONVALUE._serialized_start=450
  _MDUPDATEACTIONVALUE._serialized_end=597
  _MDUPDATEACTIONVALUE_MDUPDATEACTION._serialized_start=537
  _MDUPDATEACTIONVALUE_MDUPDATEACTION._serialized_end=597
  _ORDER._serialized_start=600
  _ORDER._serialized_end=1568
  _ORDER_MDQUOTETYPEVALUE._serialized_start=1002
  _ORDER_MDQUOTETYPEVALUE._serialized_end=1163
  _ORDER_MDQUOTETYPEVALUE_MDQUOTETYPE._serialized_start=1086
  _ORDER_MDQUOTETYPEVALUE_MDQUOTETYPE._serialized_end=1163
  _ORDER_TIMEINFORCEVALUE._serialized_start=1166
  _ORDER_TIMEINFORCEVALUE._serialized_end=1369
  _ORDER_TIMEINFORCEVALUE_TIMEINFORCE._serialized_start=1250
  _ORDER_TIMEINFORCEVALUE_TIMEINFORCE._serialized_end=1369
  _ORDER_MDENTRYTYPE._serialized_start=1371
  _ORDER_MDENTRYTYPE._serialized_end=1414
  _ORDER_IMPLIEDMARKETINDICATOR._serialized_start=1416
  _ORDER_IMPLIEDMARKETINDICATOR._serialized_end=1503
  _ORDER_EXECUTIONINSTRUCTION._serialized_start=1505
  _ORDER_EXECUTIONINSTRUCTION._serialized_end=1568
  _TRADE._serialized_start=1570
  _TRADE._serialized_end=1690
# @@protoc_insertion_point(module_scope)
